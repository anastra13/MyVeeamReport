...
If ($showSummaryBk + $showJobsBk + $showAllSessBk + $showAllTasksBk + $showRunningBk +
  ...
  #JOB Client
  $allJobs = Get-VBRJob | Where-Object { $_.Name -like $JOBNAME }
}
# Get all Backup Jobs
$allJobsBk = @($allJobs | ?{$_.JobType -eq "Backup" -or $_.JobType -eq "EpAgentBackup"})
...
If ($showSummaryEp + $showJobsEp + $showAllSessEp + $showRunningEp +
  ...
  $allJobsEp = @(Get-VBRJob | Where-Object { $_.JobType -eq "EpAgentBackup" }) 
...
$allSess = @()
If ($allJobs) {
  $allSess = Get-VBRBackupSession | Where-Object { $_.Name -like $JOBNAME }
}
# Get all Restore Sessions
$allSessResto = @()
If ($showRestoRunVM + $showRestoreVM) {
  $allSessResto = Get-VBRRestoreSession | Where-Object { $_.Name -like $JOBNAME }
}
...
$allSessEp = @()
If ($allJobsEp) {
  $allSessEp = Get-VBRComputerBackupJobSession | Where-Object { $_.Name -like $JOBNAME }
}
# Get all SureBackup Sessions
$allSessSb = @()
If ($allJobsSb) {
  $allSessSb = Get-VSBSession | Where-Object { $_.Name -like $JOBNAME }
}

# Get all Backups
$jobBackups = @()
If ($showBackupSizeBk + $showBackupSizeBc + $showBackupSizeEp) {
  $jobBackups = Get-VBRBackup | Where-Object { $_.Name -like $JOBNAME }
}
...
Function Get-VMsBackupStatus {
  ...
  Find-VBRViEntity | 
    Where-Object {$_.Type -eq "Vm" -and $_.VmFolderName -notmatch $excludefolder_regex -and $_.Path -like $entity } |
...
$bodyTop = @"
    <body>
        <center>
            <table>
                <tr>
                    <td style="width: 50%;height: 14px;border: none;background-color: #279dd1;color: White;font-size: 10px;vertical-align: bottom;text-align: left;padding: 2px 0px 0px 5px;"><img src="https://sharesecret.jn-hebergement.com/static/snappass/img/fp.png" alt="FREEPRO"></td>
...
If ($showSummaryProtect) {
  ...
  $bodySummaryProtect = $sumprotectHead + "PROTECTION" + $subHead02 + $bodySummaryProtect

   $bodySummaryProtect = $subHeadSection + "SYNTHESE" + $subHead02 + $bodySummaryProtect
}
...
If ($showSummaryBk) {
 ...
 $taux = 100 - ((($failedSessionsBk).Count * 100) / (($sessListBk).Count - ($failsSessionsBk).Count + ($failedSessionsBk).Count))
  
  $bodySummaryBk = $summaryBkHead + "SAUVEGARDES" + " / TAUX DE REUSSITE= " + [math]::Round($taux,2) +"%" + $subHead02 + $bodySummaryBk
}
...
If ($showAllSessBk) {
  If ($sessListBk.count -gt 0) {
    If ($showDetailedBk) {
      ...
      $bodyAllSessBk = $allSessBkHead + "Liste de toutes les sessions de sauvegarde" + $subHead02 + $bodyAllSessBk
    } Else {
      $arrAllSessBk = $sessListBk | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
...
If ($showAllTasksBk) {
  If ($taskListBk.count -gt 0) {
    If ($showDetailedBk) {
      ...
      $bodyAllTasksBk = $arrAllTasksBk | Sort "VM Name" | ConvertTo-HTML -Fragment
      ...
      $bodyAllTasksBk = $allTasksBkHead + "Backup Tasks (by VM name)" + $subHead02 + $bodyAllTasksBk
    } Else {
      $arrAllTasksBk = $taskListBk | Select @{Name="VM Name"; Expression = {$_.Name}},
        @{Name="Job Name"; Expression = {$_.JobSess.Name}},
        @{Name="Start Time"; Expression = {$_.Progress.StartTimeLocal}},
...
If ($showTaskWFBk) {
  If ($wfTasksBk.count -gt 0) {
    If ($showDetailedBk) {
      ...
      $bodyTaskWFBk = $taskWFBkHead + "Liste de toutes les taches de sauvegarde en WARNING ou FAILED" + $subHead02 + $bodyTaskWFBk
    } Else {
      $arrTaskWFBk = $wfTasksBk | Select @{Name="VM Name"; Expression = {$_.Name}},
        @{Name="Job Name"; Expression = {$_.JobSess.Name}},
        @{Name="Start Time"; Expression = {$_.Progress.StartTimeLocal}},
...
If ($showRestoRunVM) {
  If ($($runningResto).count -gt 0) {
    ...
    $bodyRestoRunVM = $subHead01 + "Restaurations en cours" + $subHead02 + $bodyRestoRunVM 
  }
}
...
If ($showRestoreVM) {
  If ($($completeResto).count -gt 0) {
    ...
    $bodyRestoreVM = $restoreVMHead + "Restaurations de VMs" + $subHead02 + $bodyRestoreVM 
  }
}
...
If ($showSummaryRp) {
  ...
  $bodySummaryRp = $summaryRpHead + "REPLICATIONS" + $subHead02 + $bodySummaryRp
}
...
If ($showAllSessRp) {
  If ($sessListRp.count -gt 0) {
    If ($showDetailedRp) {
      ...
      $bodyAllSessRp = $allSessRpHead + "Liste de toutes les Replications" + $subHead02 + $bodyAllSessRp
    } Else {
      $arrAllSessRp = $sessListRp | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
...
If ($showTaskWFRp) {
  If ($wfTasksRp.count -gt 0) {
    If ($showDetailedRp) {
      ...
      $bodyTaskWFRp = $taskWFRpHead + "Replications en WARNING ou FAILED" + $subHead02 + $bodyTaskWFRp
    } Else {
      $arrTaskWFRp = $wfTasksRp | Select @{Name="VM Name"; Expression = {$_.Name}},
        @{Name="Job Name"; Expression = {$_.JobSess.Name}},
        @{Name="Start Time"; Expression = {$_.Progress.StartTimeLocal}},
...
If ($showSummaryBc) {
  ...
  $bodySummaryBc = $summaryBcHead + "ARCHIVAGE - Backup Copy" + $subHead02 + $bodySummaryBc
}
...
If ($showAllSessBc) {
  If ($sessListBc.count -gt 0) {
    If ($showDetailedBc) {
      ...
      $bodyAllSessBc = $allSessBcHead + "Liste de toutes les sessions d'archivage" + $subHead02 + $bodyAllSessBc
    } Else {
      $arrAllSessBc = $sessListBc | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
        @{Name="State"; Expression = {$_.State}},
        @{Name="Start Time"; Expression = {$_.CreationTime}},
...
If ($showJobsEp) {
  If ($allJobsEp.count -gt 0) {
    $bodyJobsEp = $allJobsEp | Sort Name | Select @{Name="Job Name"; Expression = {$_.Name}},
      @{Name="NextRun"; Expression = {$_.GetScheduleOptions().NextRun}},
      @{Name="Schedule"; Expression = {$_.IsScheduleEnabled}},@{Name="Status"; Expression = {$_.GetLastState()}},
      @{Name="Target Repo"; Expression = {$_.FindTargetRepository().name}}, @{Name="LatestRun"; Expression = {$_.GetLastResult()}},
      @{Name="Last Result"; Expression = {If ($_.LastResult -eq "None"){""}Else{$_.GetLastResult()}}} | ConvertTo-HTML -Fragment
...
If ($showSummaryConfig) {
  ...
  $bodySummaryConfig = $configHead + "Sauvegarde de la configuration de Veeam" + $subHead02 + $bodySummaryConfig
}
...
If ($showProxy) {
  If ($proxyList -ne $null) {
    ...
    $bodyProxy = $proxyHead + "Serveurs Proxy " + $subHead02 + $bodyProxy
  }
}
...
If ($showRepo) {
  If ($repoList -ne $null) {
    ...
    } ElseIf ($arrRepo.status -match "Warning") {
    ...
    $bodyRepo = $repoHead + "Banques de sauvegarde" + $subHead02 + $bodyRepo
  }
}
...
If ($showLicExp) {
  ...
  $bodyLicense = $licHead + "Licence / Support" + $subHead02 + $bodyLicense
}
...
$htmlOutput += $bodyJobsBk + $bodyJobSizeBk + $bodyAllSessBk + $HTMLbreakSimple + $bodyAllTasksBk + $bodyRunningBk + $bodyTasksRunningBk + $bodySessWFBk + $bodyTaskWFBk + $bodySessSuccBk + $bodyTaskSuccBk
...
$htmlOutput += $TitleRestore + $bodyRestoRunVM + $bodyRestoreVM

If ($bodyRestoRunVM + $bodyRestoreVM) {
  $htmlOutput += $HTMLbreak
  }

$htmlOutput += $TitleReplica + $bodyJobsRp + $bodyAllSessRp + $bodyAllTasksRp + $bodyRunningRp + $bodyTasksRunningRp + $bodySessWFRp + $bodyTaskWFRp + $bodySessSuccRp + $bodyTaskSuccRp
...
$htmlOutput += $TitleBackupCopy + $bodyJobsBc + $bodyJobSizeBc + $bodyAllSessBc + $bodyAllTasksBc + $bodySessIdleBc + $bodyTasksPendingBc + $bodyRunningBc + $bodyTasksRunningBc + $bodySessWFBc + $bodyTaskWFBc + $bodySessSuccBc + $bodyTaskSuccBc
...
$htmlOutput += $TitleSystem + $bodyJobsSb + $bodyAllSessSb + $bodyAllTasksSb + $bodyRunningSb + $bodyTasksRunningSb + $bodySessWFSb + $bodyTaskWFSb + $bodySessSuccSb + $bodyTaskSuccSb
...
$htmlOutput += $bodySummaryConfig + $HTMLbreakSimple + $bodyProxy + $HTMLbreakSimple + $bodyRepo + $bodySORepo + $bodyRepoPerms + $bodyReplica + $bodyServices + $HTMLbreakSimple + $bodyLicense + $footerObj
